classdef CrewPersonImpl < handle
...
%             currentActivityIntensity = obj.CurrentActivity.Intensity;
            ...
            if length(AirInputs) < 1
                obj.O2Consumed = 0;
            else
                obj.O2Consumed = AirInputs.O2Store.take(obj.O2Needed);
            end
            % CO2
            ...
            AirOutputs.CO2Store.add(obj.CO2Produced);                   % Add CO2 to SimEnvironment
            % Food
            ...
            AirOutputs.VaporStore.add(obj.vaporProduced);                                                               % Send Vapor Produced to SimEnvironment
            % Dirty Water
...
%                         kJneeded = 106*obj.Weight + 6067*activityCoefficient;   % Incorrect version of equation used in BioSim
                ...
                caloriesNeeded = kJneeded/24/4.2;   % Convert from kJ/person/day to calories/person/hour (a more accurate conversion factor would be 4.184) (actually this should be in kcal)
            end          
