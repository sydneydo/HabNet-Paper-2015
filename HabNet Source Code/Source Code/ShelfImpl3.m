classdef ShelfImpl3 < handle
    ...
    methods
        %% Constructor
        function obj = ShelfImpl3(cropType,cropArea,Environment,GreyWaterSource,PotableWaterSource,PowerSource,BiomassOutput,growthStartTime)
            ...
            if nargin > 0
                ...
                obj.OptimalPPF = cropType.initialPPFValue;      % Set optimal PPF value
...
%                 obj.AverageCO2Concentration = cropType.initialCO2Value;
        ...
        function lightPlants(obj)
            powerToDeliver = max(min(obj.ShelfPowerLevel,obj.AveragePPF*obj.cropAreaUsed/(obj.lampEfficiency*obj.PSEfficiency)),1.4e-45);        % Always enforce this value being >= 0
...
%             litersOfWaterProduced = 0;
            ...
            if obj.CurrentCO2Concentration <= 120
...
%                 disp(obj.PPFFractionAbsorbed)
            ...
            if waterFraction < 1
                molesOfCO2ToInhale = waterFraction*obj.Crop.OPF*dailyCarbonGain*obj.cropAreaUsed/24;    % Line modified on 12/19/2014 to equal O2 produced
            else
                molesOfCO2ToInhale = obj.Crop.OPF*dailyCarbonGain*obj.cropAreaUsed/24;  % Line modified on 12/19/2014 to equal O2 produced      % [moles/hour]
            end
...
%             end
            ...
            hourlyO2MolesProduced = molesOfCO2ToInhale;      % Line added 12/19 to ensure moles of O2 produced = moles of CO2 consumed
            ...
            % O2 production)
            obj.totalO2GramsProduced = obj.totalO2GramsProduced + hourlyO2MolesProduced*32;   % [cumulative grams/hour]
                
            % Add O2 produced to Air Producer Store (note we divide by 24
            % to get the hourly rate)
            obj.MolesOfO2Exhaled = hourlyO2MolesProduced;%/24;   % [moles/hour]
            obj.AirProducerDefinition.ResourceStore.O2Store.add(hourlyO2MolesProduced);   % [moles/hour]
...
%             vaporPressureDeficit = calculateVaporPressureDeficit(obj);
            ...
            if strcmpi(obj.Crop.Type,'Erectophile')
                ...
                canopyStomatalConductance = max((1.717*obj.AirProducerDefinition.ResourceStore.temperature - 19.96 - 10.54*vaporPressureDeficit)*...
                    (netCanopyPhotosynthesis/obj.CurrentCO2Concentration),1.4E-45);
            end
